        ...
        if exist(path.series_nifti,'dir')
            ...
            for file = 1:size(niftis,1)
                if matches(series_names{series},'field_map_mag')
                    ...
                    newfile = ['sub' subID '-' series_names{series} '_' num2str(file) '.nii.gz'];
                    
                    if ~exist(newfile,'file')
                        movefile(niftis(file,:),['sub' subID '-' series_names{series} '_' num2str(file) '.nii.gz'])
                    end
...
%                     end
                    ...
                    newfile = ['sub' subID '-' series_names{series} '.nii.gz'];
                    
                    if ~exist(newfile,'file')
                        movefile(niftis(file,:),['sub' subID '-' series_names{series} '.nii.gz'])
                    end
