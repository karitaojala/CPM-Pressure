function matlabbatch = physio_batch(region,physio_name,sub_id,run_id,output_dir,physio_file,motion_file,no_motion_reg,relative_start_acquisition,no_scans_run,n_slices,onset_slice,time_slice_to_slice)
...
matlabbatch{1}.spm.tools.physio.model.output_multiple_regressors = [sub_id '-' run_id '-multiple_regressors-' region '-' version_name '.txt'];
matlabbatch{1}.spm.tools.physio.model.output_physio = [sub_id '-' run_id '-physio-' region '-' version_name '.mat'];
...
if strcmp(region,'brain')
    fmri_file = fullfile(output_dir,'..','..','mri','data',sub_id,['epi-' run_id],['ra' sub_id '-epi-' run_id '-brain.nii']);
elseif strcmp(region,'spinal')
    fmri_file = fullfile(output_dir,'..','..','mri','data',sub_id,['epi-' run_id],['a' sub_id '-epi-' run_id '-spinal_moco.nii.gz']);
end
