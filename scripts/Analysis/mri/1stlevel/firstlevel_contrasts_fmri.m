function firstlevel_contrasts_fmri(options,analysis_version,model,subj,rois)
...
for sub = subj
    ...
    for roi = rois
        ...
        %     SPM.xCon = [];
        %     save(fullfile(firstlvlpath,'SPM.mat'))
        ...
        for cong = 1:numel(model.congroups_1stlvl.names)
            ...
            clear col_ind connames conweights cond_runs
            
            congroup = model.congroups_1stlvl.names{cong};
            ...
            for run = runs
                ...
                if model.sessConcatenat % runs concatenated in the design matrix
            ...
            if model.tonicIncluded; cond_Tonic = (1+model.pmodNo(1))*(1+sum(options.basisF.hrf.derivatives)); else; cond_Tonic = 0; end %#ok<*NASGU>
            cond_Stim = 1 + model.pmodNo(2);
            if model.VASincluded; cond_VAS = 1 + model.pmodNo(3); else; cond_VAS = 0; end
            ...
            if strcmp(model.basisF,'HRF')
                ...
                elseif strcmp(congroup,'TonicPmod') % tonic pressure contrasts separated for CON and EXP conditions
                ...
                elseif strcmp(congroup,'TonicPhasicTimeConcat') % tonic pressure contrasts for concatenated runs with time = stimulus index
                ...
                elseif strcmp(congroup,'TonicPhasicPPIConcat') % tonic pressure contrasts for concatenated runs with time = stimulus index
                ...
                elseif strcmp(congroup,'NoiseReg') % noise regressors
                    ...
                    for con = 1:no_contr % Loop over contrasts for current model
                        connames{1,ci} = sprintf('NoiseReg%02d',con); % Contrast name
                        ci = ci + 1;
                    end
                    
                elseif strcmp(congroup,'RETROICOR')
                    ...
                    conweights = zeros(no_contr,no_reg);
                    col_ind = (no_cond+1):(no_cond+no_contr); % column indices of the noise regressors (after experimental condition regressors)
                    conweights(:,col_ind) = eye(no_contr,no_contr)/4;
            ...
            elseif strcmp(model.basisF,'FIR') % Finite-Impulse-Response model
            ...
            matlabbatch{cong}.spm.stats.con.spmmat = {fullfile(firstlvlpath, 'SPM.mat')};
            
            cond = 1;
            for con = 1:no_contr % Loop over contrasts for current model
                matlabbatch{cong}.spm.stats.con.consess{con}.tcon.name = connames{con,cond}; % Contrast name
                matlabbatch{cong}.spm.stats.con.consess{con}.tcon.weights = conweights(con,:); % Contrast weight
                matlabbatch{cong}.spm.stats.con.consess{con}.tcon.sessrep = conrepl; % Contrast replication across sessions
            end
            
            matlabbatch{cong}.spm.stats.con.delete = options.stats.firstlvl.contrasts.delete; % Deleting old contrast
