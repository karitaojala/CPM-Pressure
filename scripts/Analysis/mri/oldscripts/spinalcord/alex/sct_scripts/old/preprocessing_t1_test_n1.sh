...
targetdir=${subdir}/T1_test/
if [ ! -d ${targetdir} ]; then
	mkdir -p ${targetdir}	
fi

cd ${subdir}/T1_raw || exit
cp $(ls ${subdir}/T1_raw | egrep -e "sTRIO.*.nii" | tail -1) ${targetdir}t1.nii
...
if [[ ${subject} == 61 || ${subject} == 37 ]];then 
	sct_propseg -i t1.nii -c t1 -qc ~/qc_multiSubj2 -min-contrast 30
elif [[ ${subject} == 42 ]]; then
	sct_propseg -i t1.nii -c t1 -qc ~/qc_multiSubj -radius 2.3
elif [[ ${subject} == 30 || ${subject} == 65 ]]; then
	sct_propseg -i t1.nii -c t1 -qc ~/qc_multiSubj2 -radius 3.4 -min-contrast 20
elif [[ ${subject} == 29 ]]; then
	sct_propseg -i t1.nii -c t1 -qc ~/qc_multiSubj2 -radius 3 -min-contrast 30
else
	sct_deepseg_sc -i t1.nii -c t1 -qc ~/qc_multiSubj2 
fi

# find spinal vertebrae
sct_label_vertebrae -i t1.nii -s t1_seg.nii -c t1 -qc ~/qc_multiSubj2 || pbl=( "${pbl[@]}" "Sub${subject}" )
