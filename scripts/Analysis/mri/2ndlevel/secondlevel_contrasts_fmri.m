...
for con = contrasts
    ...
    if strcmp(basisF,'HRF') && ~strncmp(congroup,'NoiseROI',numel('NoiseROI'))
...
%         end
    ...
    elseif strcmp(basisF,'FIR') || strncmp(congroup,'NoiseROI',numel('NoiseROI'))
...
%     con_id = sprintf('%04d',con_ind);
    ...
    elseif strncmp(congroup,'NoiseROI',numel('NoiseROI'))
    ...
    %% Run matlabbatch
    if (strcmp(basisF,'HRF') && ~strncmp(congroup,'NoiseROI',numel('NoiseROI'))) || numel(contrasts) <= 2
...
elseif strncmp(congroup,'NoiseROI',numel('NoiseROI'))
    ...
    if strcmp(congroup,'NoiseROI-WM')
        matlabbatch{3}.spm.stats.con.consess{1}.fcon.name = 'NoiseROI-WM';
    elseif strcmp(congroup,'NoiseROI-CSF')
        matlabbatch{3}.spm.stats.con.consess{1}.fcon.name = 'NoiseROI-CSF';
    end
    matlabbatch{3}.spm.stats.con.consess{1}.fcon.weights = eye(numel(contrasts));
