...
for i = 1:size(fdir,1)
    ...
    sub_start = strfind(fname{i,:},'sub');
    new_fname = fname{i,:}(sub_start:end-2);
    %new_fname = erase(new_fname,',1');
    new_fname_full = fullfile(new_fdir,new_fname);
    %SPM.xY.P(i,1:length(new_fname_full)) = new_fname_full;
    SPM.xY.P{i} = new_fname_full;
    SPM.xY.VY(i).fname = new_fname_full;
end
