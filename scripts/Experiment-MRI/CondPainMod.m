...
%                 WaitSecs(1);
        ...
        elseif block == P.presentation.CPM.blocks % if last block, show end of the experiment screen
            ...
            % Wait for BOLD
            fprintf('Entering %ds post-experiment wait for BOLD to catch up.',P.mri.finalWait);
            WaitSecs(P.mri.finalWait); % arbitrary duration to wait out final BOLD; check if this interferes with the KbQueue somehow
            ...
            if strcmp(P.env.hostname,'stimpc1')
                ...
                P.mri.mriEnd = KbTriggerWait(P.keys.trigger,P.devices.input);
                P = LogMRITriggers(P); % save last data
