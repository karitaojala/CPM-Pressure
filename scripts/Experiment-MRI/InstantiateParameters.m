...
elseif strcmp(P.env.hostname,'isnb05cda5ba721') % own laptop
    ...
    P.path.Cogent               = fullfile(P.path.scriptBase,'..','Cogent');
elseif strcmp(P.env.hostname,'stimpc1')
    P.path.scriptBase           = cd;
    P.path.experiment           = fullfile('D:\ojala','CPM-Pressure','data',P.project.name);
    P.path.PTB                  = 'C:\toolbox\Psychtoolbox';
    P.path.Cogent               = fullfile(P.path.scriptBase,'..','Cogent');
else
    P.path.scriptBase           = cd;
...
if P.devices.arduino
    ...
    elseif strcmp(P.env.hostname,'isnb05cda5ba721') % own laptop
        ...
        P.com.arduino = 'COM14'; % CPAR: depends on PC - work laptop COM3 - experiment laptop COM5 - stimPC COM14
        P.path.cpar = fullfile(cd,'..','LabBench.CPAR-master');
        disp('stimpc');   
    end
end
...
%% MRI settings
P.mri.dummyScans        = 5;
P.mri.timeExtraVolumes  = 30;
P.mri.finalWait         = 30; 
P.mri.nTrigger          = 0;
P.mri.nRuns             = 6;
P.mri.nVolumesPerRun    = [56 232 232 232 232 56]; % including 5 dummy scans per run
P.mri.fMRIEventCount    = zeros(1,P.mri.nRuns); 
...
if P.protocol.sbOrder == 1
    limb_stim = [1 2]; % [1 2] = tonic stimulus LEFT ARM & phasic stimulus RIGHT ARM
    limb_order = [1 2]; % [1 2] = LEFT limb first, RIGHT limb second
% elseif P.protocol.sbOrder == 2
%     limb_stim = [2 1]; % [2 1] = phasic stimulus LEFT ARM & tonic stimulus RIGHT ARM
%     limb_order = [1 2]; % [1 2] = LEFT limb first, RIGHT limb second
% elseif P.protocol.sbOrder == 3
%     limb_stim = [2 1]; % [2 1] = phasic stimulus LEFT ARM & tonic stimulus RIGHT ARM
%     limb_order = [2 1]; % [2 1] = RIGHT limb first, LEFT limb second
elseif P.protocol.sbOrder == 2
    limb_stim = [1 2]; % [1 2] = tonic stimulus LEFT ARM & phasic stimulus RIGHT ARM
    limb_order = [2 1]; % [2 1] = RIGHT limb first, LEFT limb second
end
...
if limb_stim(1) == 1 % tonic/leg left - phasic/arm right
    ...
    P.presentation.armname_long_de_s = ['den ' left_de ' Arm'];
    P.presentation.armname_short_de_s = ['den ' right_de ' Arm'];
    
    P.presentation.armname_long_de_s2 = ['der ' left_de_s ' Arm'];
    P.presentation.armname_short_de_s2 = ['der ' right_de_s ' Arm'];
...
elseif limb_stim(1) == 2 % tonic right / phasic left
    ...
    P.presentation.armname_long_de_s = ['den ' right_de_s ' Arm'];
    P.presentation.armname_short_de_s = ['den ' left_de_s ' Arm'];
    
    P.presentation.armname_long_de_s2 = ['der ' right_de_s ' Arm'];
    P.presentation.armname_short_de_s2 = ['der ' left_de_s ' Arm'];
...
P.awiszus.sd  = [8 10]; % assumed population std, kPa
...
P.pain.Calibration.VASTargetsFixedPresetSteps   = [5,15,30];
P.pain.Calibration.VASTargetsVisual             = [20,30,40,50,60,70,80];
P.pain.Calibration.painThresholdPreset          = [10 10];%P.awiszus.mu; % first for tonic stimuli, second for phasic stimuli
...
P.pain.CPM.phasicStim.on                    = 1; %ones(1,P.presentation.CPM.trialsPerBlock-1); % on which trials the phasic test stimuli will be delivered to the other cuff, in addition to the tonic conditioning stimulus
...
P.pain.CPM.tonicStim.pressurePeak    = 55; % pressure at peak of the tonic stimulus (maximum), e.g. at VAS 9
...
P.pain.CPM.phasicStim.duration          = P.pain.Calibration.phasicStim.stimDuration-P.pain.CPM.phasicStim.rampDuration; % duration of phasic stimulus in seconds
...
% last trial of the block no phasic stimulus, tonic only
% these are also the trials with online VAS rating of tonic stimulus
P.presentation.CPM.contRatingInstructionDuration = 20;
P.pain.CPM.tonicRating.cycles              = 1;
P.pain.CPM.tonicRating.totalDuration       = 80;%P.pain.CPM.tonicStim.totalDuration;%P.pain.CPM.tonicStim.fullCycleDuration + 2*P.pain.CPM.tonicStim.startendRampDuration;
...
P.presentation.CPM.tonicStim.durationVAS    = 80;%P.pain.CPM.tonicRating.totalDuration;%P.pain.CPM.tonicStim.totalDuration; % Presentation duration of VAS rating scale for tonic stimuli (continous, online) when no phasic stimuli
