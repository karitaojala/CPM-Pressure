...
%             end
        ...
        countedDown = 1;
        while GetSecs < tCrossOn + (P.presentation.CPM.blockBetweenTime - P.presentation.CPM.blockBetweenText - P.mri.timeExtraVolumes) % wait the time between blocks
            tmp=num2str(SecureRound(GetSecs-tCrossOn,0));
            [abort,countedDown]=CountDown(P,GetSecs-tCrossOn,countedDown,[tmp ' ']);
            if abort; break; end
            if mod((countedDown/30), 1) == 0; fprintf('\n'); end % add line every 30 seconds
...
%         %         P = LogMRITriggers(P,run); % save last data
    ...
    while 1
        [keyIsDown, ~, keyCode] = KbCheck();
        if keyIsDown
            if find(keyCode) == P.keys.confirm
                break;
            elseif find(keyCode) == P.keys.esc
                abort = 1;
                break;
